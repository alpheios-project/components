<template>
    <div v-if="data" :id="`${data.paradigmID}`">
        <h3 class="alpheios-inflections__supp-table-title">{{data.title}}</h3>
        <div class="infl-supp-tbl__cont">
            <div class="infl-prdgm-tbl infl-prdgm-tbl--supp" :style="{ backgroundColor: bgColor }">
            ...
            <a class="infl-supp-tbl__reflink" @click="navigate">{{messages.get(INFLECTIONS_MAIN_TABLE_LINK_TEXT)}}</a>
        </div>
    </div>
</template>
<script>
